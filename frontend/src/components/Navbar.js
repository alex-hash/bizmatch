...
class App extends React.Component {
    ...
    if (this.state.user === null) {
      return (
        <div className="flex flex-wrap">
          <Link
            className="self-center  text-copy-primary font-bold py-2 px-4 border-b-0 rounded link"
      ...
      return (
        <div className="inline sm:inline xl:hidden lg:hidden md:hidden ">
          <Link to="/" className="relative flex flex-wrap">
            <button>{this.changeIcons()}</button>
  ...
  changeIcons() {
    if (document.body.classList.contains('theme-light')) {
      return <img src="https://img.icons8.com/ios/40/000000/business-network.png" />;
    } else {
      return (

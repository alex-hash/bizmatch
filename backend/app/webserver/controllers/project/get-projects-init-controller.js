...
async function getProjectsInit(req, res, next) {
  ...
  try {
    ...
    FROM project t LEFT JOIN assesment a ON t.id = a.project_id JOIN user u ON u.id=t.user_id GROUP BY t.id ORDER BY AVG(a.type) DESC, count(a.type) DESC LIMIT 3`;
